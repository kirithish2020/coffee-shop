...
document.querySelecter('#menu').onclick = () =>{
  navbar.classList.add('active');
}

document.querySelecter('#close').onclick = () =>{
  navbar.classList.remove('active');
}

document.querySelecter('#menu-btn').onclick = () =>{
...
function move(e){
    this.querySelectorAll('.move').forEach(layer =>{ 
        ...
        layer.style.transform = 'translateX(${0}px) translateY(${100}}px)'
    })
}
